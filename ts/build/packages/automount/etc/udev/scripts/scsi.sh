...
do_mounts()
{
	if [ ! -e $mtpath ] || ! cmount $mtpath; then
		...
		if [ -n "$mount_opts" ] ; then
		    case $ID_FS_TYPE in
		    ntfs )
			MT_CMD="/bin/ntfs-3g -o locale=ru_RU.utf8,silent,force /dev/$devpath $mtpath"
		    ;;
		    vfat )
        		MT_CMD="mount -t vfat -o shortname=win95,utf8,$mount_opts /dev/$devpath $mtpath"
		    ;;
		    *)
		    ...
		    ;;
		    vfat )
			MT_CMD="mount -t vfat -o shortname=win95,utf8 /dev/$devpath $mtpath"
		    ;;
		    *)
